...
int main()
{
				...
				for (int k = 0; k < sqrt(windowSize); k++) {
					for (int p = 0; p < sqrt(windowSize); p++) {
						if ((i + k) < expand_amount / 2
							...
							medianArr[median_arr_Append] = raw_image_chunk[((i + k) - (expand_amount / 2)) * (ImageWidth)+((j + p) - (expand_amount / 2))];
							median_arr_Append++;
						}
					}
				}
	...
	//Display Processing Time & Save result image
	if (rank == 0) {
		endTime = MPI_Wtime();
		if (world_size > 1)
